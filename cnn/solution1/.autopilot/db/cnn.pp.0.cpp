...
__attribute__((sdx_kernel("cnn", 0))) void cnn(fixed_type input[28][28],
...
{
#line 16 "C:/Users/Lenovo/Desktop/aaaaaaaaaaaaaaaaaaaa/cnn/solution1/csynth.tcl"
#pragma HLSDIRECTIVE TOP name=cnn
# 15 "cnn.cpp"

#line 7 "C:/Users/Lenovo/Desktop/aaaaaaaaaaaaaaaaaaaa/cnn/solution1/directives.tcl"
#pragma HLSDIRECTIVE TOP name=cnn
# 15 "cnn.cpp"
    ...
    VITIS_LOOP_19_1: for (int i = 0; i < 28; i++) {
#pragma HLS UNROLL
 VITIS_LOOP_21_2: for (int j = 0; j < 28; j++) {
#pragma HLS UNROLL factor = 2
 local_input[i + 1][j + 1] = input[i][j];
    ...
    VITIS_LOOP_28_3: for (int i = 0; i < 30; i++) {
        VITIS_LOOP_29_4: for (int j = 0; j < 30; j++) {
#pragma HLS UNROLL
 if (i == 0 || i == 29 || j == 0 || j == 29) {
    ...
    VITIS_LOOP_40_5: for (int c = 0; c < 3; c++) {
#pragma HLS UNROLL
 fixed_type (*current_kernel)[3] = kernel[c];
        fixed_type current_bias = bias[c];

        VITIS_LOOP_45_6: for (int i = 0; i < 28; i++) {

            VITIS_LOOP_47_7: for (int j = 0; j < 28; j++) {
                fixed_type sum = current_bias;
                VITIS_LOOP_49_8: for (int ki = 0; ki < 3; ki++) {
#pragma HLS UNROLL
 VITIS_LOOP_51_9: for (int kj = 0; kj < 3; kj++) {
#pragma HLS UNROLL
 sum += local_input[i + ki][j + kj] * current_kernel[ki][kj];
    ...
    int idx = 0;
    VITIS_LOOP_65_10: for (int c = 0; c < 3; c++) {
#pragma HLS UNROLL
 VITIS_LOOP_67_11: for (int i = 0; i < 28; i++) {
            VITIS_LOOP_68_12: for (int j = 0; j < 28; j++) {
#pragma HLS UNROLL
 flattened_output[idx++] = output_conv[c][i][j];
    ...
    VITIS_LOOP_77_13: for (int i = 0; i < 14; i++) {
#pragma HLS UNROLL
 fixed_type sum = b1[i];
        VITIS_LOOP_80_14: for (int j = 0; j < 2352; j++) {
            sum += W1[i][j] * flattened_output[j];
        }
        output_fc1[i] = (sum < fixed_type(0)) ? fixed_type(0) : sum;
    }


    fixed_type output_fc2[12];
    VITIS_LOOP_88_15: for (int i = 0; i < 12; i++) {
#pragma HLS UNROLL
 fixed_type sum = b2[i];
        VITIS_LOOP_91_16: for (int j = 0; j < 14; j++) {
#pragma HLS UNROLL
 sum += W2[i][j] * output_fc1[j];
        }
        output_fc2[i] = (sum < fixed_type(0)) ? fixed_type(0) : sum;
    }


    VITIS_LOOP_99_17: for (int i = 0; i < 10; i++) {
#pragma HLS UNROLL
 fixed_type sum = b3[i];
        VITIS_LOOP_102_18: for (int j = 0; j < 12; j++) {
#pragma HLS UNROLL
